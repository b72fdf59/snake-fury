...
-}
ppCell :: CellType -> L.Builder
ppCell Empty = "- "
ppCell Snake = "0 "
ppCell SnakeHead = "$ "
ppCell Apple = "X "

renderBoard' :: BoardInfo -> (Point -> CellType) -> L.Builder
renderBoard' boardInfo getCellType =
  foldl'
    ( \acc i ->
        acc
          <> foldl'
            (\iAcc j -> iAcc <> ppCell (getCellType (i, j)))
            ""
            [1 .. width boardInfo]
          <> "\n"
    )
...
render boardInfo renderState =
  let ppBoard =
   ...
   in ppScore (score renderState) <> ppBoard <> "\n"
